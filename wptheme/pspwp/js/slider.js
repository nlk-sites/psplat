...
// Original code from www.mclelun.com
...
function initializeRotator() {
    jqb_vTotal = j(".jqb_slides").children().size() -1;
    jqb_intInterval = setInterval(jqb_fnLoop, jqb_vDuration);
			
    j("#jqb_object").find(".jqb_slide").each(function(i) {
        jqb_tmp = ((i - 1)*boxWidth) - ((jqb_vCurrent -1)*boxWidth);
        j(this).animate({
            "left": jqb_tmp+"px"
        },0);
    });
    ...
    j('.jqb_slide').mouseenter(function() {
        clearInterval(jqb_intInterval);
        clearInterval(restartInterval);
        jqb_vIsPause = true;
    });
    j('.jqb_slide').mouseleave(function() {
        clearInterval(restartInterval);
        restartInterval = setInterval(restartChange, 5000);
        jqb_vIsPause = false;
    });


    j("li.infoDefault").mouseenter(function(){
        if(jqb_vCurrent != j(this).index())
        {
            
            while(j(this).index() != jqb_vCurrent) {
                jqb_vGo = 1;
                jqb_fnChange();
            }
...
function jqb_fnLoop(){
    if(!jqb_vIsPause) {
        j('#infoBox_' + jqb_vCurrent).removeClass('infoHighlighted');
        if(jqb_vGo == 1){
        ...
        j('#infoBox_' + jqb_vCurrent).addClass('infoHighlighted');
        j("#jqb_object").find(".jqb_slide").each(function(i) {
...
jQuery(document).ready(function() {
    ...
    var homeSlidesContainer = homeSlideshowWrap.find('div.slider-container div.slider-container-inner');
	...
	pagerLaPercentW = pagerElPercentW + 1;
	
    // Make the highlight bar the same width as each pager element
    highlightBar.css('width', pagerLaPercentW+'%');
